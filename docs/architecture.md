...
**Where the templates live in the source tree.** The bundled `.claude/agents/*.md`, `.claude/commands/*.md`, and `.claude/settings.json` that `remory init` writes into the user's data directory are sourced from `src/remory/` (the package data). They are not in the repo's `.claude/`. They are not user-editable: a future refresh mechanism will rewrite them from the package copy on upgrade, and `remory doctor` notes drift in the meantime. The reasoning is in [ADR-0011](./adr/0011-bundled-prompts.md).
...
Two distinct delivery patterns appear in the table below. Schemas and sleep-pipeline prompt templates are **loaded from the installed package at runtime** — they are never copied onto the user's disk, so a Remory upgrade picks them up automatically. Subagent definitions, slash command definitions, and hook scripts are **materialised to disk by `remory init`** into `<data_dir>/.claude/` so that the `claude` CLI can find them; the originals still live in the package, and a future refresh mechanism rewrites the on-disk copies. The asymmetry is deliberate: the package-loaded artefacts are Python's, so Python can read them directly; the materialised artefacts are Claude Code's, and Claude Code expects them on the filesystem.
